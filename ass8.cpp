...
int main() {
    ...
    if((state.WBEND.instruction).substr(0,6).compare("000000")==0 && state.WBEND.instruction!="00000000000000000000000000000000"){
      ...
      state.WBEND.instruction=newState.MEMWB.instruction;
      i++;
      cout<<"executing sw: Value stored."<<"\n";
    }
    ...
    if((state.MEMWB.instruction).substr(0,6).compare("000000")==0){
      ...
      state.dataMemory[newState.EXMEM.address]=decToBinary32(state.registers[newState.EXMEM.destReg]);
    }
    ...
    if((state.EXMEM.instruction).substr(0,6).compare("000000")==0){
      if((state.EXMEM.instruction).substr(26,6).compare("100000")==0){	//add
        //the following code looks after the hazard caused by the use of destreg by the next instruction
        if(newState.IDEX.readReg1==newState.EXMEM.destReg && newState.EXMEM.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalling\n";
        	state.cycles+=2;
        	state.EXMEM.instruction = "00000000000000000000000000000000";
        	state.EXMEM.destReg = -1;
        	state.EXMEM.address = 0;
        	state.MEMWB.nextPc = 0;
        	state.EXMEM.value= -1;
        	i--;
        	newState= state;
        	continue;
        }
        else if(newState.IDEX.readReg2==newState.EXMEM.destReg && newState.EXMEM.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalling\n";
        	state.cycles +=2 ;
        	state.EXMEM.instruction = "00000000000000000000000000000000";
        	state.EXMEM.destReg = -1;
        ...
        state.EXMEM.destReg=newState.IDEX.destReg;
      	...
      	if(newState.IDEX.readReg2==newState.EXMEM.destReg && newState.EXMEM.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalling\n";
        	state.cycles+=2;
        	state.EXMEM.instruction = "00000000000000000000000000000000";
        	state.EXMEM.destReg = -1;
      ...
      state.EXMEM.destReg=newState.IDEX.destReg;
    	...
    	if(newState.IDEX.readReg1 == newState.EXMEM.destReg && newState.EXMEM.instruction != "00000000000000000000000000000000"){
        	...
        	state.EXMEM.value= -1;
      		...
      		newState = state;
      		continue;
    	} else if (newState.IDEX.destReg == newState.EXMEM.destReg && newState.EXMEM.instruction != "00000000000000000000000000000000"){
    		cout<<"stalling for sw\n";
      		state.cycles+=2;
        	state.EXMEM.instruction = "00000000000000000000000000000000";
        	state.EXMEM.destReg = -1;
      ...
      state.EXMEM.value=newState.IDEX.offset;
    	...
    	if(newState.IDEX.readReg1 == newState.EXMEM.destReg || newState.IDEX.readReg2 == newState.EXMEM.destReg){
    		cout<<"Stalling\n";
    		state.cycles+=2;
        	state.EXMEM.instruction = "00000000000000000000000000000000";
        	state.EXMEM.destReg = -1;
    ...
    if((state.IDEX.instruction).substr(0,6).compare("000000")==0){
    	//for add sub srl and sll we need to look into data hazards
      if((state.IDEX.instruction).substr(26,6).compare("100000")==0){		//for add
        ...
        //the follwing code looks after the hazard caused by the use of the destreg in the next to next line
        if(state.IDEX.readReg1==newState.MEMWB.destReg && state.MEMWB.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalled instruction "<<i-3<<" requires the read register 1\n";
        	state.cycles++;
        }
        else if(state.IDEX.readReg2==newState.MEMWB.destReg && state.MEMWB.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalled instruction "<<i-3<<" requires the read register 2\n";
        	state.cycles++;
        }
        //this type of hazards are handeled
        ...
        state.IDEX.readReg2=binaryToDec((state.IDEX.instruction).substr(11,5));
        state.IDEX.destReg=binaryToDec((state.IDEX.instruction).substr(16,5));
        if(state.IDEX.readReg1==newState.EXMEM.destReg && state.MEMWB.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalled instruction "<<i-3<<" requires the read register 1\n";
        	state.cycles++;
        }
        else if(state.IDEX.readReg2==newState.EXMEM.destReg && state.MEMWB.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalled instruction "<<i-3<<" requires the read register 2\n";
        	state.cycles++;
        }
      }
        ...
        state.IDEX.readReg2=binaryToDec((state.IDEX.instruction).substr(11,5));
        state.IDEX.destReg=binaryToDec((state.IDEX.instruction).substr(16,5));
        if(state.IDEX.readReg2==newState.EXMEM.destReg && state.MEMWB.instruction!="00000000000000000000000000000000"){
        	cout<<"Stalled instruction "<<i-3<<" requires the read register.\n";
        	state.cycles++;
        }
